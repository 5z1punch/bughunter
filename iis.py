...
import time
monkey.patch_all()
logpath = "temp/iis_"+time.strftime("%Y-%m-%d_%H:%M", time.localtime(time.time()))
...
def main():
	res = get_from_database("select address,port from blog.scan_scan_port where product like '%iis%'")
	res = list(set(res))
